...
import {
  SelectErrorSlice,
  setError,
} from "../../store/Reducer/Slices/ErrorSlice/errorSlice";
...
const MieiTicket = () => {
  ...
  const [backFromDetails, setBackfromDetails] = useState(
    location.state?.prevstate.previousPath == "/dettaglio"
  );
  ...
  const sortElementi = useCallback(
    (datiStraordinari = null) => {
      const datiClone = datiStraordinari
        ? [...datiStraordinari]
        : [...elementi];
                  ...
                  <button
                    style={{
                      border:
                           ...
                           notif.mieiTicketNotifNumber.includes(e.$id)
                          ? "2px solid red"
                          : "none",
                    }}
    ...
    switch (intestazione) {
      ...
      case "Messaggi": {
        ...
        return messaggi.length > 0 ? (
          <button
            onClick={() => {
              const listaMessaggi = messaggi.map((messaggio) => {
                return `${messaggio}\n`;
              });
  ...
  return (
    <div>
      ...
      <ConditionalRenderer showContent={showContent}>
          ...
          <table>
            ...
            <tbody>
              {perTabella.length > 0 && (
                <Paginator
                  elemPerPagina={5}
                  getNumeroPagina={getNumeroPagina}
                  paginaCorrente={
                    location.state !== null
                      ? location.state.prevstate.previousState.page
                      : 1
                  }
                >
                  {perTabella.map((riga) => {
        ...
        {filter !== "" && elementi.length === 0 && (
          <p>Non ci sono ticket con questo filtro</p>
        )}
      </ConditionalRenderer>
    </div>
  );
